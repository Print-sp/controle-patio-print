...
function canonicalText(value) {
    return String(value || '')
        .trim()
        .toLowerCase()
        .replace(/Ã¡|Ã¢|Ã£|Ã¤|á|â|ã|à|ä/g, 'a')
        .replace(/Ã§|ç/g, 'c')
        .replace(/Ã©|Ãª|Ã¨|Ã«|é|ê|è|ë/g, 'e')
        .replace(/Ã­|Ã¬|Ã®|Ã¯|í|ì|î|ï/g, 'i')
        .replace(/Ã³|Ã´|Ãµ|Ã¶|ó|ô|õ|ò|ö/g, 'o')
        .replace(/Ãº|Ã¹|Ã»|Ã¼|ú|ù|û|ü/g, 'u')
        .replace(/[^a-z0-9]+/g, ' ')
        .trim();
}
...
app.put('/api/vehicles/:id', requireAuth, async (req, res) => {
    ...
    const canEditDates = req.session.user.role === 'admin' || req.session.user.username?.toLowerCase() === 'bandeirantes' || (req.session.user.yards || []).includes('P�tio Bandeirantes');
    if (updates.yard) {
        const allowedYards = getUserAllowedYards(req.session.user.username);
    ...
    try {
        if (isProduction) {
            await pool.query(`UPDATE vehicles SET plate = COALESCE($1::TEXT, plate), type = COALESCE($2::TEXT, type), yard = COALESCE($3::TEXT, yard), base = COALESCE($4::TEXT, base), baseDestino = COALESCE($5::TEXT, baseDestino), manager = COALESCE($6::TEXT, manager), chassis = COALESCE($7::TEXT, chassis), status = COALESCE($8::TEXT, status), maintenance = COALESCE($9::BOOLEAN, maintenance), hasAccident = COALESCE($10::BOOLEAN, hasAccident), documentIssue = COALESCE($11::BOOLEAN, documentIssue), sascarStatus = COALESCE($12::TEXT, sascarStatus), maintenanceCategory = COALESCE($13::TEXT, maintenanceCategory), notes = COALESCE($14::TEXT, notes), entregar_diversos = COALESCE($15::BOOLEAN, entregar_diversos), entregar_correios = COALESCE($16::BOOLEAN, entregar_correios), entregue = COALESCE($17::BOOLEAN, entregue), entreguePara = COALESCE($18::TEXT, entreguePara), entryTime = CASE WHEN $19::TEXT IS NOT NULL AND ${canEditDates} THEN $19::TIMESTAMP ELSE entryTime END, exitTime = CASE WHEN $20::TEXT IS NOT NULL AND ${canEditDates} THEN $20::TIMESTAMP ELSE exitTime END, updatedAt = CURRENT_TIMESTAMP, updatedBy = $21::TEXT WHERE id = $22::INTEGER`,
                [updates.plate || null, updates.type || null, updates.yard || null, updates.base || null, updates.baseDestino || null, updates.manager || null, updates.chassis || null, updates.status || null, updates.maintenance !== undefined ? Boolean(updates.maintenance) : null, updates.hasAccident !== undefined ? Boolean(updates.hasAccident) : null, updates.documentIssue !== undefined ? Boolean(updates.documentIssue) : null, updates.sascarStatus || null, updates.maintenanceCategory || null, updates.notes || null, updates.entregarDiversos !== undefined ? Boolean(updates.entregarDiversos) : null, updates.entregarCorreios !== undefined ? Boolean(updates.entregarCorreios) : null, updates.entregue !== undefined ? Boolean(updates.entregue) : null, updates.entreguePara || null, canEditDates && updates.entryTime ? updates.entryTime : null, canEditDates && updates.exitTime ? updates.exitTime : null, req.session.user.username, id]);
        } else {
            db.prepare(`UPDATE vehicles SET plate = ?, type = ?, yard = ?, base = ?, baseDestino = ?, manager = ?, chassis = ?, status = ?, maintenance = ?, hasAccident = ?, documentIssue = ?, sascarStatus = ?, maintenanceCategory = ?, notes = ?, entregar_diversos = ?, entregar_correios = ?, entregue = ?, entreguePara = ?, entryTime = ?, exitTime = ?, updatedAt = ?, updatedBy = ? WHERE id = ?`).run(
...
app.put('/api/vehicles/:id/status', requireAuth, async (req, res) => {
    ...
    try {
        if (isProduction) {
            ...
            db.prepare(`UPDATE vehicles SET status = ?, maintenance = ?, updatedAt = ? WHERE id = ?`).run(status, (status === 'Em manuten��o' || status === 'Borracharia') ? 1 : 0, new Date().toISOString(), id);
        }
